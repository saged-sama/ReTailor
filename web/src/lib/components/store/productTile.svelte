...
<div class="flex flex-col rounded-md bg-base-100 h-full w-64 shadow-xl xl:hover:scale-105 transition-all duration-300">
    <a class="relative w-full overflow-hidden h-64 bg-black flex items-center justify-end"
        ...
        <img src={imageurl} alt={product.name} class="xl:hover:scale-110 transition-all duration-300 z-10">
    ...
    <div class="flex text-xs flex-col px-2 py-1 gap-2">
        <a class=" text-sm h-11 text-ellipsis overflow-hidden font-bold font-platypi xl:hover:underline" href={`/store/product/${product.id}`} title={product.name}>{product.name}</a>
        <div class="flex flex-col gap-1 bg-primary-content border-2 border-gray-800 p-2 rounded-md">
            ...
            <div class="flex gap-1"><h1 class="font-bold font-platypi">Tailor:</h1> <a class="xl:hover:text-primary" href={`/tailor/${product.tailorId}`}> {product.firstName} {product.lastName}</a></div>
